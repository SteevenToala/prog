...
try {
    ...
    if ($result_vehicle->num_rows > 0) {
        ...
        // Actualizar solo el estado del alquiler
        $sql = "UPDATE alquileres SET estado = 'Activo' WHERE id = ?";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("i", $id_rent);
        if (!$stmt->execute() || $stmt->affected_rows <= 0) {
            throw new Exception('Error al actualizar el estado del alquiler o no se afectó ninguna fila');
        }

        // Obtener detalles del vehículo actualizado
